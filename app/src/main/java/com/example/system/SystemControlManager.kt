...
class SystemControlManager(private val context: Context) {
    ...
    /**
     * Now actually flips the Wi-Fi/Bluetooth switch instead of only opening the settings page
     * and stopping there. Needs MAX's Accessibility Service to be enabled (Settings tab in the
     * app) — without it, this still opens the page but can't tap the toggle for you, since no
     * regular app is allowed to silently flip radios since Android 6.
     */
    suspend fun toggleSystemSetting(setting: String): String {
        val lower = setting.lowercase()
        val desiredOn: Boolean? = when {
            lower.contains(" on") || lower.contains("chalu") || lower.contains("start") || lower.contains("enable") -> true
            lower.contains(" off") || lower.contains("band") || lower.contains("bandh") || lower.contains("stop") || lower.contains("disable") -> false
            else -> null // no explicit direction given — just toggle whatever it currently is
        }
        ...
        return when {
            lower.contains("wifi") || lower.contains("wi-fi") -> {
                ...
                applyToggleViaAccessibility(desiredOn, "Wi-Fi")
            }
            lower.contains("bluetooth") -> {
                ...
                context.startActivity(intent)
                applyToggleViaAccessibility(desiredOn, "Bluetooth")
            }
    ...
    /** Waits for the settings page to draw, then taps the real on-screen switch via the
     * Accessibility Service. Returns an honest status message either way — never claims the
     * toggle happened if it didn't. */
    private suspend fun applyToggleViaAccessibility(desiredOn: Boolean?, label: String): String {
        val service = MaxAccessibilityService.instance
        if (service == null) {
            return "Opened $label settings, but I need Accessibility Service enabled (Settings tab in MAX) to actually flip the switch for you, Sir."
        }
        kotlinx.coroutines.delay(700)
        val success = service.setMainSwitchState(desiredOn)
        return if (success) {
            when (desiredOn) {
                true -> "$label turned ON, Sir."
                false -> "$label turned OFF, Sir."
                null -> "$label toggled, Sir."
            }
        } else {
            "Opened $label settings, but couldn't find the switch to tap — you may need to flip it manually this time, Sir."
        }
    }
    ...
    /** Places the call directly (ACTION_CALL) instead of just opening the dialer, since the
     * user explicitly wants every action fully completed, not left for a manual tap. Needs
     * CALL_PHONE granted — already requested at runtime in MainActivity. Falls back to
     * ACTION_DIAL (dialer pre-filled, safe) if permission isn't granted, rather than crashing. */
            ...
            is ActionConfirmationPolicy.Decision.Execute -> {
                ...
                if (hasCallPermission) {
                    ...
                    try {
                        ...
                        "Calling ${contact.name} now, Sir."
                    } catch (e: Exception) {
                        "Unable to place the call to ${contact.name}, Sir."
                    }
                } else {
                    ...
                    try {
                        ...
                        "Need Call permission to dial ${contact.name} directly — dialer opened, tap call to connect, Sir."
                    } catch (e: Exception) {
    ...
    /** Opens WhatsApp with the message pre-filled via the official per-chat deep link, then —
     * if MAX's Accessibility Service is enabled — taps the Send button so the message is
     * actually delivered, not just typed and left for the user to send. */
        ...
        return when (decision) {
            ...
            is ActionConfirmationPolicy.Decision.Execute -> {
                ...
                try {
                    ...
                    val service = MaxAccessibilityService.instance
                    if (service != null) {
                        // WhatsApp needs a moment to open the chat with the text pre-filled
                        // before the Send button exists on screen to tap.
                        kotlinx.coroutines.delay(1600)
                        val sent = service.clickElementByText("Send")
                        if (sent) {
                            "Message sent to ${contact.name} on WhatsApp, Sir."
                        } else {
                            "Opened WhatsApp chat with ${contact.name}, message pre-filled, but couldn't tap Send automatically — please tap it, Sir."
                        }
                    } else {
                        "Opened WhatsApp chat with ${contact.name}, message pre-filled — enable Accessibility Service in Settings so I can send it for you next time, Sir."
                    }
                } catch (e: Exception) {
    ...
    /**
     * Opens YouTube's search, then — if MAX's Accessibility Service is enabled — taps the
     * first video result so it actually starts playing, instead of just leaving the search
     * results list open. There's no official "auto-play the first result" intent, so this
     * drives the real on-screen tap the same way a finger would (see
     * [MaxAccessibilityService.clickFirstVideoResult] for exactly how that tap is chosen).
     */
    suspend fun openYouTubeSearch(query: String): Boolean {
        if (query.isBlank()) return false
        ...
        val openedApp = try { context.startActivity(appIntent); true } catch (e: Exception) { false }
        if (openedApp) {
            val service = MaxAccessibilityService.instance
            if (service != null) {
                // Cold-starting the YouTube app + running its search is slower than a settings
                // page, so this waits longer before looking for something to tap.
