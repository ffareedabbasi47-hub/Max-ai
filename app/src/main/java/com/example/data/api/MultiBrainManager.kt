...
class MultiBrainManager(private val context: Context) {
    ...
    suspend fun runGeminiDiagnostic(apiKey: String = ""): GeminiDiagnosticResult {
        // No key is baked into the APK any more, so a blank key means "use whichever key the
        // user configured in Settings" (custom key / rotation slots).
        val key = apiKey.trim().ifBlank { getGeminiApiKeys().firstOrNull().orEmpty() }
        return diagnosticService.testGeminiConnectivity(key)
    }
